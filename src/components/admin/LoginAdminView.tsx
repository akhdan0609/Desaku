...
export const LoginAdminView: React.FC<LoginAdminProps> = ({ onLoginSuccess, onNavigate }) => {
  ...
  const [password, setPassword] = useState('desa2026');
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (checkAdminLogin(username, password)) {
      ...
      setErrorMsg('Username atau Kata Sandi salah! (Petunjuk: admin / desa2026)');
    }
  };
      ...
      <div className="max-w-md w-full bg-white rounded-3xl p-8 sm:p-10 border border-slate-200 shadow-xl space-y-6">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="p-3 bg-slate-50 rounded-xl text-[11px] text-slate-500 space-y-0.5 border border-slate-100">
            ...
            <p>Username: <code className="bg-slate-200 px-1 rounded font-bold text-slate-800">admin</code></p>
            <p>Password: <code className="bg-slate-200 px-1 rounded font-bold text-slate-800">desa2026</code></p>
          </div>

...
export const LayananPernikahanView: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    let jenisLabel = 'Surat Pengantar Nikah & Formulir N1-N4';
    let prefix = 'NIKAH';
    ...
    const newDoc = {
      ...
      keperluan: `Layanan Pernikahan: Rencana Akad ${formData.tanggalAkad} di ${formData.lokasiAkad}`,
  ...
  const modelForms = [
    { code: 'Model N1', name: 'Surat Pengantar Nikah dari Kepala Desa', desc: 'Menerangkan biodata lengkap calon mempelai warga desa.' },
    { code: 'Model N2', name: 'Surat Permohonan Kehendak Nikah', desc: 'Surat resmi permohonan pendaftaran ke KUA Cijeruk.' },
    { code: 'Model N3', name: 'Surat Persetujuan Mempelai', desc: 'Pernyataan kerelaan kedua calon mempelai tanpa paksaan.' },
    { code: 'Model N4', name: 'Surat Izin Orang Tua / Wali', desc: 'Persetujuan resmi dari orang tua kandung atau wali sah.' }
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            <div className="space-y-1">
              <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md inline-block mb-1 ${
                activeTab === 'pengantar-nikah' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'
              }`}>
                Paket Berkas Lengkap
              </span>
              <h3 className={`font-bold text-sm ${activeTab === 'pengantar-nikah' ? 'text-emerald-900' : 'text-slate-900'}`}>
                Pengantar Nikah & Form N1 - N4
              </h3>
              <p className="text-xs text-slate-500 leading-relaxed">
                Pengajuan berkas terpadu rekomendasi nikah untuk dibawa ke KUA Cijeruk.
              </p>
            </div>
            <div className="pt-3 flex items-center gap-1.5 text-xs font-semibold text-emerald-700">
              <span>Buka Formulir Pengajuan</span>
              <span>→</span>
            </div>
          </button>

          <button
            onClick={() => { setActiveTab('formulir-n1-n4'); setSubmittedCode(null); }}
            className={`p-5 rounded-2xl border text-left transition duration-200 flex flex-col justify-between ${
          ...
          >
            <div className="space-y-1">
              <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md inline-block mb-1 ${
                activeTab === 'formulir-n1-n4' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'
              }`}>
                Struktur Formulir
              </span>
              <h3 className={`font-bold text-sm ${activeTab === 'formulir-n1-n4' ? 'text-emerald-900' : 'text-slate-900'}`}>
                Panduan Formulir N1 s.d N4
              </h3>
              <p className="text-xs text-slate-500 leading-relaxed">
                Penjelasan detail fungsi setiap lembar formulir model N1, N2, N3, dan N4.
              </p>
            </div>
            <div className="pt-3 flex items-center gap-1.5 text-xs font-semibold text-emerald-700">
              <span>Lihat Rincian Formulir</span>
              <span>→</span>
            </div>
          </button>

          <button
            onClick={() => { setActiveTab('belum-nikah'); setSubmittedCode(null); }}
            className={`p-5 rounded-2xl border text-left transition duration-200 flex flex-col justify-between ${
          ...
          >
            <div className="space-y-1">
              <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md inline-block mb-1 ${
                activeTab === 'belum-nikah' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'
              }`}>
                Surat Keterangan
              </span>
              <h3 className={`font-bold text-sm ${activeTab === 'belum-nikah' ? 'text-emerald-900' : 'text-slate-900'}`}>
        ...
        {/* Tab 2: Rincian Formulir N1-N4 Info */}
        {activeTab === 'formulir-n1-n4' && (
          <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm space-y-6">
            <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              <FileText className="w-5 h-5 text-emerald-700" />
              <span>Rincian Berkas Model N1, N2, N3, dan N4</span>
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {modelForms.map(form => (
                <div key={form.code} className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
                  ...
                  <h4 className="font-bold text-slate-900 text-sm">{form.name}</h4>
                  <p className="text-xs text-slate-600 leading-relaxed">{form.desc}</p>
                </div>
              ))}
            </div>
            <div className="pt-2 text-center">
              <button
                onClick={() => setActiveTab('pengantar-nikah')}
                className="bg-emerald-700 hover:bg-emerald-800 text-white font-bold px-6 py-2.5 rounded-xl text-xs transition"
              >
                Lanjut ke Formulir Pengajuan Nikah Online
              </button>
            </div>
          </div>
        )}
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-7 border border-slate-200 shadow-sm space-y-6">
              <div className="border-b border-slate-100 pb-3">
                <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                  ...
                  <span>Persyaratan Pengantar Nikah</span>
                </h3>
            ...
            <div className="lg:col-span-2 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
              ...
              {submittedCode ? (
                <div className="bg-emerald-50 border border-emerald-300 rounded-3xl p-8 text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-xl font-bold text-emerald-950">Pengajuan Nikah Berhasil Diterima!</h3>
                    <p className="text-xs text-emerald-800">
                      Silakan catat kode registrasi pengantar nikah Anda:
                    </p>
                  </div>

                  <div className="bg-white border-2 border-dashed border-emerald-600 py-3 px-6 rounded-2xl inline-block">
                    <span className="text-xs text-slate-500 block">Nomor Registrasi Pernikahan</span>
                  ...
                  <p className="text-xs text-slate-600 max-w-md mx-auto">
                    Petugas Desa Warung Menteng akan mencetak bundel Formulir N1, N2, N3, dan N4 lengkap dengan tandatangan Kepala Desa.
                  </p>
